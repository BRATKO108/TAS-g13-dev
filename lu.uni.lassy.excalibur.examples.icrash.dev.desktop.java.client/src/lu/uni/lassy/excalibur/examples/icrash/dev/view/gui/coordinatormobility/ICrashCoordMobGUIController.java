 ...
 */
public class ICrashCoordMobGUIController extends AbstractAuthGUIController {
	/*
	...
	*/
    ...
    /** The textfield for entering Time to send. */
    @FXML
    private TextField txtfldTimeSend;
    ...
    /** The button that allows a user to report time of react. */
    @FXML
    private Button bttnReportTime;
     ...
     */
    @FXML
    void bttnReportTime_OnClick(ActionEvent event) {
    	reportOnTime();
    }
	...
	/**
	 * Runs the function that will allow the current user to report TIME on the selected crisis.
	 */
	private void reportOnTime(){
		System.out.println("Working");
		CtCrisis crisis = (CtCrisis)getObjectFromTableView(tblvwCrisis);
		if (crisis != null)
		{
			try {
				if (!userController.reportOnTime(crisis.id.value.getValue(), txtfldTimeSend.getText()).getValue())
					showWarningMessage("Unable to send time of crisis", "Unable to send time of crisis, please try again");
			} catch (ServerOfflineException | ServerNotBoundException e) {
				showServerOffLineMessage(e);
			} catch (IncorrectFormatException e) {
				showWarningIncorrectInformationEntered(e);
