...
public class IcrashSystemImpl extends UnicastRemoteObject implements
	...
	/* (non-Javadoc)
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#oeSetCrisisTime(lu.uni.lassy.excalibur.examples.icrash.dev.java.system.types.primary.DtCrisisID)
	 */
	public synchronized  PtBoolean oeSetCrisisTime(DtCrisisID aDtCrisisID, String time)
			throws RemoteException {
		try{
			//PreP1
			isSystemStarted();
			...
			if (currentRequestingAuthenticatedActor instanceof ActCoordinator) {
				...
				for (CtAlert theAlert : getAlertsByCrisis(theCrisis))
				{
					...
					while(enumKey.hasMoreElements()){
						...
						if (aCtAlert.equals(theAlert)){
							CtHuman theHuman = assCtAlertCtHuman.get(aCtAlert);
							if (!theHuman.isTime(time).getValue())
								log.error("Unable to message a communication company about the crisis update");
						}
					}
				}
			...
			log.error("Exception in oeSetCrisisTime..." + e);
		}
